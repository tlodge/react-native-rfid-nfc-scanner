...
const NFC_OUT_OF_RANGE = '__NFC_OUT_OF_RANGE';
...
if (Platform.OS == "ios") {
    ...
    DeviceEventEmitter.addListener(NFC_OUT_OF_RANGE, (res) => {
        _enabled = true;
        _loading = false;
        _status = "ready";
    });
...
let _registerToEvents = () => {
    if (Platform.OS == "ios") {
        ...
        try{
            ...
            DeviceEventEmitter.addListener(NFC_OUT_OF_RANGE,_notifyRangeListeners);
...
let _notifyRangeListeners = ()=>{
    for(let _listener in _rangeListeners){
        _rangeListeners[_listener]();
    }
}
...
NFC.addRangeListener = (name, callback)=>{
    if (callback){
        _rangeListeners[name] = callback;
    }
}
...
NFC.writeTag = (message)=>{
    ReactNativeNFC.writeTag(message);
};
...
NFC.removeAllListeners = () => {
    if (Platform.OS == "ios") {
        ...
        try{
            DeviceEventEmitter.removeAllListeners(NFC_DISCOVERED);
            DeviceEventEmitter.removeAllListeners(NFC_ERROR);
            DeviceEventEmitter.removeAllListeners(NFC_OUT_OF_RANGE);
        }catch(androidErr){
            console.log("androidErr", androidErr);
        }
    }
...
export class NfcRfidScanner {
    ...
    addRangeListener = function(name, listenerCallback){
        if(NFC.isEnabled()){
            NFC.addRangeListener(name, listenerCallback);
        }
    }
